...
pgfe_otp_t _10pow(uint8_t p);

inline pgfe_otp_t _10pow(uint8_t p) {
    return DIGITS_POW[p];
}
...
pgfe_otp_t pgfe_hotp_generic(
    ...
    reverse(counter_h, counter_h + counter_len - 1);

    pgfe_hmac(alg, secret, secret_length, counter_h, counter_len, out_hash);
    result = dynamically_truncate(out_hash, digest_size);

    if (digit_c < PGFE_OTP_DIGIT_MAX_LIMIT) {
        result %= _10pow(digit_c);
    }
